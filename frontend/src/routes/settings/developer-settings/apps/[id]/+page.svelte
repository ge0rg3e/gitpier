<script lang="ts">
	...
	import { gitpierApps, type GitPierApp, type AppPrivateKey, type AppInstallation } from '$lib/api/client';
	...
	async function handleDeleteApp() {
		...
		try {
			...
			goto('/settings/developer-settings/apps');
		} catch (e: any) {
			saveError = e.message;
			deletingApp = false;
		}
...
<svelte:head>
	<title>{app ? `${app.name} - GitPier App settings` : 'GitPier App'}</title>
</svelte:head>
	...
	{:else if app}
		...
		{#if newSecret}
			<div class="mb-6 rounded-md border border-amber-800/40 bg-amber-900/20 p-4">
				...
				<p class="text-xs text-amber-400/80 mb-3">This secret will never be shown again. Store it securely.</p>
				<div class="flex items-center gap-2">
		...
		<a
			href="/settings/developer-settings/apps/{appId}/permissions"
		...
		<div class="rounded-md border border-border bg-card p-5 mb-6">
			<div class="flex items-center justify-between mb-4">
				<div>
					...
					<p class="text-xs text-muted-foreground mt-0.5">Used to sign JWTs for server-to-server authentication. You can have up to 10 active keys.</p>
				</div>
			...
			{#if newPrivateKey}
				<div class="mb-4 rounded-md border border-emerald-800/40 bg-emerald-900/20 p-4">
					<div class="flex items-start gap-2 mb-2">
						...
						<p class="text-sm font-semibold text-emerald-300">Download your private key</p>
					</div>
					<p class="text-xs text-emerald-400/80 mb-3">
						This private key will not be stored and cannot be recovered. Download it now.
						<br />Fingerprint: <code class="font-mono">{newKeyFingerprint}</code>
					</p>
					<Button type="button" variant="outline" size="sm" onclick={downloadKey}>
						Download <code class="ml-1 text-xs">.pem</code>
					</Button>
				</div>
			{/if}
						...
						<div class="flex items-center gap-3 px-4 py-3">
							<div class="flex-1 min-w-0">
								<p class="text-xs text-foreground">
									{inst.account_type === 'org' ? 'Organization' : 'User'} #{inst.account_id}
								</p>
		...
		<form onsubmit={handleSave} class="space-y-6">
			...
			<div class="rounded-md border border-border bg-card p-6 space-y-4">
				...
				<div class="space-y-2 pt-2 border-t border-border">
					<label class="flex items-center gap-2 cursor-pointer">
						<input type="checkbox" bind:checked={expireUserTokens} class="rounded border-border" />
						<span class="text-sm text-foreground">Expire user authorization tokens</span>
					</label>
					<label class="flex items-center gap-2 cursor-pointer">
						<input type="checkbox" bind:checked={requestUserAuth} class="rounded border-border" />
						<span class="text-sm text-foreground">Request user authorization during installation</span>
					</label>
					<label class="flex items-center gap-2 cursor-pointer">
						<input type="checkbox" bind:checked={enableDeviceFlow} class="rounded border-border" />
						<span class="text-sm text-foreground">Enable Device Flow</span>
					</label>
				</div>
			</div>
