<script lang="ts">
	...
	import { ArrowLeft, Loader, AlertTriangle, Check, ShieldCheck } from '@lucide/svelte';
	...
	// Detect if any permission was elevated (could require re-approval)
...
<div class="max-w-2xl">
	...
	<div class="flex items-center gap-3 mb-6">
		<a href="/settings/developer-settings/apps/{appId}" class="text-muted-foreground hover:text-foreground transition-colors">
	...
	{:else if app}
		<!-- Re-approval warning when there are installations and permissions are being elevated -->
		...
		<form onsubmit={handleSave}>
			...
			{#if hasElevatedPerms && installationCount > 0}
				<div class="mb-5 rounded-md border border-amber-800/40 bg-amber-900/10 px-4 py-3 flex gap-3">
					<ShieldCheck class="h-4 w-4 text-amber-400 shrink-0 mt-0.5" />
					<p class="text-xs text-amber-400">
						You've elevated one or more permissions. After saving, the {installationCount}
						existing {installationCount === 1 ? 'installation' : 'installations'} will be prompted to approve the new permissions the next time the app is configured.
					</p>
				</div>
			{/if}
			...
			<div class="flex items-center gap-3">
				<Button type="submit" disabled={saving || !hasChanges}>
					{#if saving}
						<Loader class="h-3.5 w-3.5 animate-spin mr-1.5" />
					{/if}
					{saving ? 'Saving…' : 'Save permissions'}
				</Button>
				{#if hasChanges}
					<span class="text-xs text-muted-foreground">You have unsaved changes</span>
				{/if}
			</div>
		</form>
	{/if}
