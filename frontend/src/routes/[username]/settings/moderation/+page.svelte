<script lang="ts">
	...
	import { moderation, type ModerationPolicy, type Organization } from '$lib/api/client';
	import { authStore } from '$lib/stores/auth.svelte';
	import { Button } from '$lib/components/ui/button/index.js';
	import { Switch } from '$lib/components/ui/switch/index.js';
	import { Badge } from '$lib/components/ui/badge/index.js';
	import { Separator } from '$lib/components/ui/separator/index.js';
	import { Shield, UserX, Hash, Loader, Plus, Trash2, Users, GitPullRequest, MessageSquare, GitCommitHorizontal, Clock, Activity, Info } from '@lucide/svelte';
	...
	const api = $derived(moderation.org(handle));
	...
	$effect(() => {
		if (!ctx.loading && ctx.isOwner && !policy && !error) {
			loadPolicy();
		}
	});
	...
	<div class="space-y-6">
		...
		<div class="flex items-center gap-3">
			...
			<div>
				...
				<p class="text-sm text-muted-foreground">Rules that apply to all repositories in <strong>{ctx.org?.display_name || handle}</strong>.</p>
			</div>
		</div>

		<div class="flex items-start gap-2 rounded-md border border-blue-800/40 bg-blue-900/10 px-4 py-3 text-sm text-blue-400">
			<Info class="h-4 w-4 mt-0.5 shrink-0" />
			<p>These rules are inherited by all organization repositories unless overridden per repository.</p>
		</div>
		...
		<section class="rounded-lg border border-border bg-card p-5 space-y-4">
			<h2 class="text-sm font-semibold text-foreground flex items-center gap-2">
				<Shield class="h-4 w-4 text-muted-foreground" />Interaction locks
			</h2>
			<Separator />
			<div class="space-y-3">
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-xs text-muted-foreground">Disable issue creation across all org repos.</p>
					</div>
				...
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-xs text-muted-foreground">Disable PR creation across all org repos.</p>
					</div>
				...
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-xs text-muted-foreground">Disable commenting across all org repos.</p>
					</div>
				...
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-xs text-muted-foreground">Prevent external users from pushing to org repos.</p>
					</div>
		...
		<section class="rounded-lg border border-border bg-card p-5 space-y-4">
			...
			<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
				<div>
					<label for="org-max-issues" class="block text-xs font-semibold text-muted-foreground mb-1">Max issues per user / day</label>
					<input
						id="org-max-issues"
						type="number"
						min="0"
				...
				<div>
					<label for="org-max-prs" class="block text-xs font-semibold text-muted-foreground mb-1">Max PRs per user / day</label>
					<input
						id="org-max-prs"
						type="number"
						min="0"
				...
				<div>
					<label for="org-max-comments" class="block text-xs font-semibold text-muted-foreground mb-1">Max comments per user / day</label>
					<input
						id="org-max-comments"
						type="number"
						min="0"
		...
		<section class="rounded-lg border border-border bg-card p-5 space-y-4">
			<h2 class="text-sm font-semibold text-foreground flex items-center gap-2">
				<Activity class="h-4 w-4 text-muted-foreground" />Account requirements
			</h2>
			<Separator />
			<div>
				<label for="org-min-account-age" class="block text-xs font-semibold text-muted-foreground mb-1">Minimum account age (days, 0 = no requirement)</label>
				<input
					id="org-min-account-age"
					type="number"
					min="0"
				...
				{#if policy.min_account_age_days > 0}
					<p class="mt-1 text-xs text-muted-foreground">= {Math.floor(policy.min_account_age_days / 30)} months {policy.min_account_age_days % 30} days</p>
				{/if}
			</div>
			<div class="flex items-start gap-3">
				<Switch bind:checked={policy.require_min_activity} id="org-req-activity" />
				<div>
					<label for="org-req-activity" class="text-sm font-medium text-foreground cursor-pointer">Require minimum activity</label>
			...
			{#if policy.require_min_activity}
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pl-9">
					<div>
						<label for="org-min-commits" class="block text-xs font-semibold text-muted-foreground mb-1">Min commits</label>
						<input
							id="org-min-commits"
							type="number"
							min="0"
					...
					<div>
						<label for="org-min-contributions" class="block text-xs font-semibold text-muted-foreground mb-1">Min contributions</label>
						<input
							id="org-min-contributions"
							type="number"
							min="0"
		...
		<section class="rounded-lg border border-border bg-card p-5 space-y-4">
			...
			<form onsubmit={blockUser} class="flex gap-2 flex-wrap">
				<input
					...
					class="h-8 flex-1 min-w-40 rounded-md border border-border bg-background px-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary"
				/>
				<input
					type="text"
			...
			{#if blockError}<p class="text-xs text-red-400">{blockError}</p>{/if}
						...
						<li class="flex items-center justify-between rounded-md border border-border px-3 py-2">
							<div class="flex items-center gap-2 min-w-0">
								...
								<div class="min-w-0">
									...
									{#if bu.reason}<p class="text-xs text-muted-foreground truncate">{bu.reason}</p>{/if}
								</div>
							</div>
		...
		<section class="rounded-lg border border-border bg-card p-5 space-y-4">
			...
			<Separator />
			<p class="text-xs text-muted-foreground">Content containing these words will be rejected across all org repositories.</p>
			...
			{#if keywordError}<p class="text-xs text-red-400">{keywordError}</p>{/if}
