<script lang="ts">
	...
	import { oauthApps, type Organization } from '$lib/api/client';
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		error = '';
		try {
			const res = await oauthApps.createOrgApp(handle, {
				name,
				description,
			...
			goto(`/${handle}/settings/oauth-apps/${res.app.id}?new=1#secret=${res.client_secret}`);
		} catch (e: any) {
			error = e.message;
		} finally {
			loading = false;
...
<svelte:head>
	<title>Register OAuth App · {handle}</title>
</svelte:head>
	...
	<div class="rounded-md border border-border bg-card p-6">
		<form onsubmit={handleSubmit} class="space-y-5">
			...
			<div class="flex items-center gap-3 pt-2 border-t border-border">
				...
				<Button variant="ghost" type="button" onclick={() => goto(`/${handle}/settings/oauth-apps`)}>Cancel</Button>
			</div>
		</form>
	</div>
</div>
