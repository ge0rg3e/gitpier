	...
	<div class="mx-auto max-w-7xl">
		...
		{#if isMobileSidebarOpen}
			...
			>
				...
				<OrgSidebar {handle} active={activeTab} isOwner={ctx.isOwner || ctx.loading} mobile />
			</div>
		{/if}

		<div class="flex flex-col gap-6 lg:flex-row lg:gap-8">
			<OrgSidebar {handle} active={activeTab} isOwner={ctx.isOwner || ctx.loading} />
			<div class="flex-1 min-w-0">
				{#if ctx.loading}
					<div class="max-w-xl">
						<div class="mb-6 h-7 w-56 rounded-md border border-border bg-card"></div>
						<div class="mb-5 rounded-md border border-border bg-card p-4">
							<div class="h-16 w-full rounded-md border border-border bg-secondary/40"></div>
						</div>
						<div class="mb-4 h-9 w-full rounded-md border border-border bg-card"></div>
						<div class="mb-4 h-24 w-full rounded-md border border-border bg-card"></div>
						<div class="mb-4 grid grid-cols-2 gap-4">
							<div class="h-9 rounded-md border border-border bg-card"></div>
							<div class="h-9 rounded-md border border-border bg-card"></div>
						</div>
						<div class="mb-8 h-9 w-40 rounded-md border border-border bg-card ml-auto"></div>
						<div class="h-px w-full bg-secondary mb-8"></div>
						<div class="h-24 w-full rounded-md border border-border bg-card"></div>
					</div>
				{:else}
					{@render children()}
				{/if}
