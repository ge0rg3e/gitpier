<script lang="ts">
	...
	import { ArrowLeft, Loader, AlertTriangle, Check, Boxes, ExternalLink } from '@lucide/svelte';
	...
	let orgRepos = $state<Array<{ id: number; name: string; full_name: string }>>([]);
	let selectedRepoIds = $state<Set<number>>(new Set());
	let loadingRepos = $state(false);

	$effect(() => {
		if (!ctx.loading && !ctx.isOwner) {
			goto(`/${handle}/settings/installed-apps`);
		}
	});
	...
	async function loadOrgRepos() {
		loadingRepos = true;
		try {
			// Use the repos API to get org repos
			const res = await fetch(`/api/orgs/${handle}/repos`).then((r) => r.json());
			orgRepos = res.repos ?? [];
		} catch {
			orgRepos = [];
		} finally {
	...
	async function handleSelectionChange(val: 'all' | 'selected') {
		repoSelection = val;
		if (val === 'selected' && orgRepos.length === 0) {
			await loadOrgRepos();
		}
	}
...
<svelte:head>
	<title>{inst?.app?.name ?? 'App'} installation · {handle}</title>
</svelte:head>

<div class="max-w-2xl">
	<div class="flex items-center gap-3 mb-6">
		<a href="/{handle}/settings/installed-apps" class="text-muted-foreground hover:text-foreground transition-colors">
		...
		<div class="rounded-md border border-border bg-card p-5 mb-6">
			<div class="flex items-start justify-between gap-4 mb-4">
				<div>
					...
					<p class="text-xs text-muted-foreground mt-0.5">What this app can access in your organization</p>
				</div>
				...
				<div class="space-y-2">
					<label class="flex items-start gap-3 cursor-pointer">
						...
						<div>
							...
							<span class="text-xs text-muted-foreground">This applies to all current and future repositories owned by {handle}.</span>
						</div>
					</label>
					<label class="flex items-start gap-3 cursor-pointer">
				...
				{#if repoSelection === 'selected'}
					<div class="pt-2 border-t border-border">
						...
						{:else if orgRepos.length === 0}
							<p class="text-xs text-muted-foreground">No repositories found in this organization.</p>
						{:else}
		...
		<div class="rounded-md border border-red-800/40 bg-card p-5">
			...
			{#if confirmUninstall}
				<p class="text-sm text-foreground mb-3">
					Are you sure you want to uninstall <strong>{inst.app?.name}</strong> from <strong>{handle}</strong>? The app will immediately lose access to all repositories and organization data.
				</p>
			...
			{:else}
				<p class="text-xs text-muted-foreground mb-3">
					Uninstalling this app will immediately revoke its access to all repositories and data in <strong>{handle}</strong>.
				</p>
