<script lang="ts">
	...
	import { Loader, ArrowLeft, Copy, RotateCcw, Trash2, CheckCircle } from '@lucide/svelte';
	...
	$effect(() => {
		if (!ctx.loading && !ctx.isOwner) {
			goto(`/${handle}/settings/oauth-apps`);
		}
	});
	...
	async function handleSave(e: SubmitEvent) {
		...
		saveError = '';
		try {
			const data = await oauthApps.update(appId, { name, description, homepage_url: homepageURL, callback_url: callbackURL, logo_url: logoURL, enable_device_flow: enableDeviceFlow });
			app = data.app;
		} catch (e: any) {
			saveError = e.message;
		} finally {
...
<svelte:head>
	<title>{app?.name ?? 'OAuth App'} · {handle}</title>
</svelte:head>

<div class="max-w-2xl">
	<div class="flex items-center gap-3 mb-6">
		<a href="/{handle}/settings/oauth-apps" class="text-muted-foreground hover:text-foreground transition-colors">
	...
	{:else if app}
				...
				<div>
						...
						<div class="rounded-md border border-yellow-800/40 bg-yellow-900/10 px-4 py-3 mb-2">
							...
							<div class="flex items-center gap-2">
								...
								<button
									...
									title="Copy"
								>
									{#if copiedSecret}<CheckCircle class="h-3.5 w-3.5 text-green-400" />{:else}<Copy class="h-3.5 w-3.5" />{/if}
								</button>
							</div>
						</div>
					{:else}
					...
					{#if showRegenConfirm}
						<div class="rounded-md border border-red-800/40 bg-red-900/10 p-3 mt-2">
							<p class="text-xs text-red-300 mb-3">Regenerating will invalidate the current secret immediately.</p>
		...
		<form onsubmit={handleSave} class="rounded-md border border-border bg-card p-5 space-y-5">
			...
			<div>
				<label for="app-name" class="block text-sm font-semibold text-foreground mb-1.5">Application name <span class="text-red-400">*</span></label>
				<input
					id="app-name"
					type="text"
					bind:value={name}
			...
			<div>
				<label for="homepage-url" class="block text-sm font-semibold text-foreground mb-1.5">Homepage URL <span class="text-red-400">*</span></label>
				<input
					id="homepage-url"
					type="url"
			...
			<div>
				<label for="callback-url" class="block text-sm font-semibold text-foreground mb-1.5">Authorization callback URL <span class="text-red-400">*</span></label>
				<input
					id="callback-url"
					type="url"
