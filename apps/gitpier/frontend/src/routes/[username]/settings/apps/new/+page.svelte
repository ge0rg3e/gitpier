<script lang="ts">
	...
	import { gitpierApps, type Organization } from '$lib/api/client';
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		try {
			...
			const res = await gitpierApps.createOrgApp(handle, {
				name,
				description,
			...
			goto(`/${handle}/settings/apps/${res.app.id}?new=1#secret=${res.client_secret}`);
		} catch (e: unknown) {
			error = (e as { message?: string }).message ?? 'Failed to create app';
		} finally {
			loading = false;
		}
	}
...
<svelte:head>
	<title>Register a new GitPier App · {handle}</title>
</svelte:head>
	...
	<form onsubmit={handleSubmit} class="space-y-8">
		...
		<div class="rounded-md border border-border bg-card p-6 space-y-5">
			...
			<div>
				...
				/>
				<p class="text-xs text-muted-foreground mt-1">The name must be globally unique. It will be slugified and used as the app's identifier.</p>
			</div>
			...
			<div>
				...
				<p class="text-xs text-muted-foreground mt-1 ml-5">Public apps can be installed by any GitPier user or organization.</p>
			</div>
		</div>
		...
		<div class="rounded-md border border-border bg-card p-6 space-y-5">
			...
			<p class="text-xs text-muted-foreground">The full URL to redirect to after a user authorizes an installation.</p>
			...
			<div class="space-y-3 pt-2 border-t border-border">
				<label class="flex items-start gap-2 cursor-pointer">
					...
					<div>
						...
						<span class="text-xs text-muted-foreground">Provides a refresh token when access tokens expire. Recommended.</span>
					</div>
				</label>
				<label class="flex items-start gap-2 cursor-pointer">
					<input type="checkbox" bind:checked={requestUserAuth} class="rounded border-border mt-0.5" />
					<div>
						<span class="text-sm font-semibold text-foreground block">Request user authorization during installation</span>
						<span class="text-xs text-muted-foreground">Users will be prompted to authorize the app when they install it.</span>
					</div>
				</label>
				<label class="flex items-start gap-2 cursor-pointer">
					...
					<div>
						...
						<span class="text-xs text-muted-foreground">Allow this app to authorize users via the Device Flow (for CLI tools).</span>
					</div>
				</label>
			</div>
		</div>
		...
		<div class="rounded-md border border-border bg-card p-6 space-y-5">
			...
			<label class="flex items-start gap-2 cursor-pointer">
				...
				<div>
					...
					<span class="text-xs text-muted-foreground">Redirect users to the Setup URL after they update the installation.</span>
				</div>
			</label>
		</div>
		...
		<div class="rounded-md border border-border bg-card p-6 space-y-5">
			...
			{#if webhookActive}
				<div>
					<label for="webhook-url" class="block text-sm font-semibold text-foreground mb-1.5">Webhook URL</label>
					<input
						id="webhook-url"
						type="url"
		...
		<div class="rounded-md border border-border bg-card p-6 space-y-5">
			<div>
				...
				<p class="text-xs text-muted-foreground mt-3">Select the minimum permissions your app needs.</p>
			</div>
		...
		<div class="rounded-md border border-border bg-card p-6 space-y-4">
			...
			<label class="flex items-start gap-3 cursor-pointer">
				...
				<div>
					...
					<span class="text-xs text-muted-foreground">Only allow this app to be installed on your organization or accounts you own.</span>
				</div>
			</label>
			<label class="flex items-start gap-3 cursor-pointer">
				...
				<div>
					...
					<span class="text-xs text-muted-foreground">Allow this app to be installed by any user or organization.</span>
				</div>
			</label>
		</div>
