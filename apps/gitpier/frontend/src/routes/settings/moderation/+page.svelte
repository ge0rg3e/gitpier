<script lang="ts">
	...
	import { moderation, type ModerationPolicy } from '$lib/api/client';
...
<div class="max-w-2xl space-y-6">
	<div class="flex items-center gap-3">
		...
		<div>
			<h1 class="text-xl font-semibold text-foreground">Global moderation</h1>
			<p class="text-sm text-muted-foreground">These rules apply to all your repositories by default.</p>
		</div>
	</div>
	...
	<div class="flex items-start gap-2 rounded-md border border-blue-800/40 bg-blue-900/10 px-4 py-3 text-sm text-blue-400">
		...
		<p>Rules set here are automatically inherited by all your repositories unless overridden at the repository level.</p>
	</div>
	...
	{:else if policy}
			...
			<div class="space-y-3">
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-xs text-muted-foreground">Disable issue creation across all your repos.</p>
					</div>
				...
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-xs text-muted-foreground">Disable PR creation across all your repos.</p>
					</div>
				...
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-xs text-muted-foreground">Disable commenting across all your repos.</p>
					</div>
				...
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-xs text-muted-foreground">Prevent external users from pushing.</p>
					</div>
		...
		<section class="rounded-lg border border-border bg-card p-5 space-y-4">
			...
			<div class="flex items-start gap-3">
				<Switch bind:checked={policy.require_min_activity} id="req-activity" />
				<div>
					<label for="req-activity" class="text-sm font-medium text-foreground cursor-pointer">Require minimum activity</label>
			...
			{#if policy.require_min_activity}
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pl-9">
					<div>
						<label class="block text-xs font-semibold text-muted-foreground mb-1">Min commits</label>
						<input
							type="number"
							min="0"
					...
					<div>
						<label class="block text-xs font-semibold text-muted-foreground mb-1">Min contributions</label>
						<input
							type="number"
							min="0"
		...
		<section class="rounded-lg border border-border bg-card p-5 space-y-4">
			...
			<Separator />
			<p class="text-xs text-muted-foreground">Content containing these words will be rejected across all your repositories.</p>
